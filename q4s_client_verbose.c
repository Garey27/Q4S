...
void store_parameters(type_q4s_session *q4s_session, type_q4s_message *q4s_message) {
  ...
  pthread_mutex_lock(&mutex_print);
	...
	if ((q4s_session)->session_id < 0) {
		...
		if (fragment = strstr(copy_header, "Session-Id")) {
			...
			strcpy(copy_header, header);  // restore copy of header
			pthread_mutex_lock(&mutex_print);
			fprintf(stderr,"Session ID stored: %d\n", (q4s_session)->session_id);
			pthread_mutex_unlock(&mutex_print);
		} else if (fragment = strstr(copy_body, "o=")) {  // if Session ID is in the body
			fragment = strstr(fragment, " ");  // moves to the beginning of the value
			char *string_id;
  ...
  // If there is a Expires parameter in the header
	if (fragment = strstr(copy_header, "Expires")){
		...
		strcpy(copy_header, header);  // restores copy of header
		pthread_mutex_lock(&mutex_print);
		fprintf(stderr,"Expiration time stored: %d\n", (q4s_session)->expiration_time);
		pthread_mutex_unlock(&mutex_print);
	}
  ...
  if (strcmp(copy_start_line, "PING q4s://www.example.com Q4S/1.0") == 0) {
		...
		if (fragment = strstr(copy_header, "Timestamp")){
			...
			strcpy(copy_header, header);  // restores copy of header
			pthread_mutex_lock(&mutex_print);
			fprintf(stderr,"Server timestamp stored: %s\n", (q4s_session)->server_timestamp);
			pthread_mutex_unlock(&mutex_print);
		}
		// If there is a Sequence Number parameter in the header
		if (fragment = strstr(copy_header, "Sequence-Number")){
			...
			strcpy(copy_header, header);  // restores copy of header
			pthread_mutex_lock(&mutex_print);
			fprintf(stderr,"Sequence number of server stored: %d\n", (q4s_session)->seq_num_server);
			pthread_mutex_unlock(&mutex_print);
		}
		...
		if (fragment = strstr(copy_header, "Measurements")){
			...
			if (strcmp(s_latency," ") != 0) {
				...
				pthread_mutex_lock(&mutex_print);
				fprintf(stderr,"Latency measure of server stored: %d\n", (q4s_session)->latency_measure_server);
	      pthread_mutex_unlock(&mutex_print);
			}
			...
			if (strcmp(s_jitter," ") != 0) {
				...
				pthread_mutex_lock(&mutex_print);
				fprintf(stderr,"Jitter measure of server stored: %d\n", (q4s_session)->jitter_measure_server);
	      pthread_mutex_unlock(&mutex_print);
			}
      ...
      if (strcmp(s_pl," ") != 0) {
				...
				pthread_mutex_lock(&mutex_print);
				fprintf(stderr,"Packetloss measure of server stored: %.2f\n", (q4s_session)->packetloss_measure_server);
	      pthread_mutex_unlock(&mutex_print);
			}
	...
	if (strcmp(copy_start_line, "BWIDTH q4s://www.example.com Q4S/1.0") == 0) {
      ...
      if (strcmp(s_bw," ") != 0) {
				...
				pthread_mutex_lock(&mutex_print);
				fprintf(stderr,"Bandwidth measure of server stored: %d\n", (q4s_session)->bw_measure_server);
	      pthread_mutex_unlock(&mutex_print);
			}
			memset(fragment, '\0', strlen(fragment));
			strcpy(copy_header, header);  // restores copy of header
	...
	if (strcmp(copy_start_line, "Q4S/1.0 200 OK") == 0) {
		...
		if (fragment = strstr(copy_header, "Sequence-Number")){
			...
			strcpy(copy_header, header);  // restores copy of header
			pthread_mutex_lock(&mutex_print);
			fprintf(stderr,"Sequence number confirmed: %d\n", (q4s_session)->seq_num_confirmed);
			pthread_mutex_unlock(&mutex_print);
		}
		...
		if (fragment = strstr(copy_header, "Stage")){
			...
			strcpy(copy_header, header);  // restores copy of header
			pthread_mutex_lock(&mutex_print);
			fprintf(stderr,"We are in Stage: %d\n", (q4s_session)->stage);
			pthread_mutex_unlock(&mutex_print);
		}
	}
  // If there is a QoS level parameter in the body
    ...
    fragment = fragment + 12;  // moves to the beginning of the value
		...
		strcpy(copy_body, body);  // restores copy of body
		pthread_mutex_lock(&mutex_print);
		fprintf(stderr,"QoS levels stored: %d/%d\n", (q4s_session)->qos_level[0],
  ...
  // If there is an alert pause parameter in the body
	if (fragment = strstr(copy_body, "a=alert-pause:")){
		...
		strcpy(copy_body, body);  // restores copy of body
		pthread_mutex_lock(&mutex_print);
		fprintf(stderr,"Alert pause stored: %d\n", (q4s_session)->alert_pause);
		pthread_mutex_unlock(&mutex_print);
	}
    ...
    pthread_mutex_lock(&mutex_print);
		fprintf(stderr,"Interval between Q4S PING stored for client (NEGOTIATION): %d\n", (q4s_session)->ping_clk_negotiation_client);
    pthread_mutex_unlock(&mutex_print);
    ...
    pthread_mutex_lock(&mutex_print);
		fprintf(stderr,"Interval between Q4S PING stored for server (NEGOTIATION): %d\n", (q4s_session)->ping_clk_negotiation_server);
    pthread_mutex_unlock(&mutex_print);
    ...
    pthread_mutex_lock(&mutex_print);
		fprintf(stderr,"Interval between Q4S PING stored (CONTINUITY): %d\n", (q4s_session)->ping_clk_continuity);
    pthread_mutex_unlock(&mutex_print);
    ...
    pthread_mutex_lock(&mutex_print);
		fprintf(stderr,"Interval between Q4S BWIDTH stored: %d\n", (q4s_session)->bwidth_clk);
    pthread_mutex_unlock(&mutex_print);
		char *window_size_lj;
    ...
    pthread_mutex_lock(&mutex_print);
		fprintf(stderr,"Window size for latency and jitter stored: %d\n", (q4s_session)->window_size_latency_jitter);
    pthread_mutex_unlock(&mutex_print);
		char *window_size_pl;
		strtok(NULL, ",");
    ...
    pthread_mutex_lock(&mutex_print);
		fprintf(stderr,"Window size for packet loss stored: %d\n", (q4s_session)->window_size_packetloss);
    pthread_mutex_unlock(&mutex_print);
		memset(fragment, '\0', strlen(fragment));
		strcpy(copy_body, body);  // restores copy of body
  ...
  // If there is an latency parameter in the body
	if (fragment = strstr(copy_body, "a=latency:")){
		...
		strcpy(copy_body, body);  // restores copy of body
		pthread_mutex_lock(&mutex_print);
		fprintf(stderr,"Latency threshold stored: %d\n", (q4s_session)->latency_th);
		pthread_mutex_unlock(&mutex_print);
	}
  ...
  // If there is an jitter parameter in the body
    ...
    fragment = fragment + 9;  // moves to the beginning of the value
		...
		strcpy(copy_body, body);  // restores copy of body
		pthread_mutex_lock(&mutex_print);
		fprintf(stderr,"Jitter thresholds stored: %d/%d\n", (q4s_session)->jitter_th[0],
  ...
  // If there is an bandwidth parameter in the body
    ...
    fragment = fragment + 12;  // moves to the beginning of the value
		...
		strcpy(copy_body, body);  // restores copy of body
		pthread_mutex_lock(&mutex_print);
		fprintf(stderr,"Bandwidth thresholds stored: %d/%d\n", (q4s_session)->bw_th[0],
		  (q4s_session)->bw_th[1]);
		pthread_mutex_unlock(&mutex_print);
	    ...
	    for (int j = 1; j < sizeof(ms_per_message); j++) {
			...
			pthread_mutex_lock(&mutex_print);
			fprintf(stderr,"\nMessages fract per second: %f", messages_fract_per_ms);
			fprintf(stderr,"\nMessages int per second: %d", messages_int_per_ms);
			fprintf(stderr,"\nBwidth threshold: %d", (q4s_session)->bw_th[0]);
			fprintf(stderr,"\nBwidth clk: %d", (q4s_session)->bwidth_clk);
			fprintf(stderr,"\nMessages per ms: %d", (q4s_session)->bwidth_messages_per_ms);
			fprintf(stderr,"\nMs per message (%d): %d\n", 1, (q4s_session)->ms_per_bwidth_message[0]);
			for (int i = 1; i < sizeof(ms_per_message); i++) {
				if (ms_per_message[i] > 0) {
					fprintf(stderr,"\nMs per message (%d): %d\n", i+1, (q4s_session)->ms_per_bwidth_message[i]);
				} else {
					break;
				}
			}
  ...
  // If there is an packetloss parameter in the body
    ...
    fragment = fragment + 13;  // moves to the beginning of the value
		...
		strcpy(copy_body, body);  // restores copy of body
		pthread_mutex_lock(&mutex_print);
		fprintf(stderr,"Packetloss thresholds stored: %.*f/%.*f\n", 2,
...
void update_latency(type_q4s_session *q4s_session, int latency_measured) {
  ...
  // Calculates mean value using statistical median formula
	...
	pthread_mutex_lock(&mutex_print);
	fprintf(stderr,"Updated value of latency measure: %d\n", (q4s_session)->latency_measure_client);
	pthread_mutex_unlock(&mutex_print);
}
...
void update_jitter(type_q4s_session *q4s_session, int elapsed_time) {
  ...
  // Calculates mean value using arithmetic mean formula
	...
	pthread_mutex_lock(&mutex_print);
	fprintf(stderr,"Updated value of jitter measure: %d\n", (q4s_session)->jitter_measure_client);
  pthread_mutex_unlock(&mutex_print);
}
...
void update_packetloss(type_q4s_session *q4s_session, int lost_packets) {
  ...
  (q4s_session)->packetloss_measure_client = ((float) num_losses / (float) window_size);
	pthread_mutex_lock(&mutex_print);
	fprintf(stderr,"Updated value of packetloss measure: %.2f\n", (q4s_session)->packetloss_measure_client);
  pthread_mutex_unlock(&mutex_print);
}
...
void *bwidth_reception_timeout() {
	...
	pthread_mutex_lock(&mutex_print);
  fprintf(stderr,"\nInterval of Q4S BWIDTH reception has started\n");
	pthread_mutex_unlock(&mutex_print);

	delay(bwidth_clk);

	pthread_mutex_lock(&mutex_print);
  fprintf(stderr,"\nInterval of Q4S BWIDTH reception has finished\n");
	pthread_mutex_unlock(&mutex_print);
	...
	while(1) {
		...
		if (ms_iterated >= bwidth_clk || num_messages_sent >= num_messages_to_send) {
		  ...
		  if (result < 0) {
			...
			pthread_mutex_lock(&mutex_print);
	    fprintf(stderr,"\nA whole burst of Q4S BWIDTH has been sent: %d messages\n", num_messages_sent);
			pthread_mutex_unlock(&mutex_print);

			pthread_mutex_lock(&mutex_print);
			fprintf(stderr,"Interval for delivery elapsed: %d\n", elapsed_time);
			pthread_mutex_unlock(&mutex_print);

			pthread_mutex_lock(&mutex_print);
			fprintf(stderr,"Theoric interval for delivery: %d\n", bwidth_clk);
			pthread_mutex_unlock(&mutex_print);

			pthread_mutex_lock(&mutex_print);
			fprintf(stderr,"Ms passed: %d\n", us_passed_total/1000);
			pthread_mutex_unlock(&mutex_print);

			pthread_mutex_lock(&mutex_print);
			fprintf(stderr,"Ms to subtract: %d\n", us_to_subtract/1000);
			pthread_mutex_unlock(&mutex_print);
...
void Update (fsm_t* fsm) {
    ...
    if ((&q4s_session)->jitter_th[1] > 0) {
			...
			if (num_ping % 2 == 0 && num_ping > 0) {
				...
				pthread_mutex_lock(&mutex_print);
				fprintf(stderr,"Elapsed time stored: %d\n", elapsed_time);
				pthread_mutex_unlock(&mutex_print);
